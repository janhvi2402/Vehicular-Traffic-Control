...
ca_avg/evaluate.py
===================
Full evaluation protocol for a trained ca_avg (average-reward
critic-actor, Panda & Bhatnagar 2025) checkpoint, run over multiple
episodes with different random seeds, against the same two baselines used
for DQN/PPO/AC-CA (see environment/eval_common.py) -- random and swept
fixed-cycle -- so results are directly comparable across all five
algorithms now in this project (DQN, PPO, AC, CA, and this CA-avg).

Usage:
    python -m ca_avg.evaluate --checkpoint runs/ca_avg_run1/actor_final.pt \\
        --episodes 10 --out runs/ca_avg_run1/eval

    # generalization test on the 3x3 network (same pattern as the others):
    python -m ca_avg.evaluate --checkpoint runs/ca_avg_run1/actor_final.pt \\
        --net_file sumo_3x3_network/grid3x3.net.xml \\
        --route_file sumo_3x3_network/routes3x3.rou.xml \\
        --episodes 2 --out runs/ca_avg_run1/eval_3x3
"""
...
def ca_avg_policy_fn(actor, device, greedy=True):
    def _policy(obs):
...
def main():
    ...
    print(f"Evaluating CA-avg over {args.episodes} episodes (seeds {seeds[0]}..{seeds[-1]})")

    algo_rows = run_policy_over_seeds(env, ca_avg_policy_fn(actor, device, greedy=not args.stochastic), seeds)
    print("CA-avg done.")
    ...
    print("\n" + "=" * 72)
    print(f"{'metric':<22s}{'CA-avg':>15s}{'Fixed-cycle':>17s}{'Random':>15s}")
    print("-" * 72)
    ...
    plot_comparison(summaries, args.out, algo_name="CA-avg")
